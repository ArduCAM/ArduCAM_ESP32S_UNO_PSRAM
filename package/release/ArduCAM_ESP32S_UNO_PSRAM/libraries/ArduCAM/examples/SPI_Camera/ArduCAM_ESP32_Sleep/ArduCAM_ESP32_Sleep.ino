...
// of the library with a supported camera modules, and can run on ArduCAM ESP32 UNO.
// This demo was made for Omnivision  2MP/5MP sensor and will test the function of ESP Sleep. 
...
// 5. After above operations,the ESP32 mode will go into sleep mode for ten seconds. 
 ...
 Serial.println(length, DEC);
    ...
    if (length == 0 ) //0 kb
  {
    Serial.println(F("Size is 0."));
      ...
      if(!file) break;  //this is a new file name
      k++;
      sprintf((char*)path,"/%05d.jpg",k);
    }
    file = fs.open(path, FILE_WRITE);  //open it by writting mode.
   i = 0;
   myCAM.CS_LOW();
